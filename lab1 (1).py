...
class NeuralNetwork_2Layer():
    def __init__(self, inputSize, outputSize, neuronsPerLayer, learningRate, activation):
        ...
        for i in range(epochs):
            ...
            for xBatch, yBatch in zip(xBatches, yBatches):
                if self.activation == "sigmoid":
                    ...
                    l1out = self.__relu(np.dot(xBatch, self.W1))
                    l2out = self.__relu(np.dot(l1out, self.W2))
                    l2e = (l2out - yBatch)/len(yBatch)
...
def trainModel(data):
    ...
    if ALGORITHM == "guesser":
        ...
        custom_model = NeuralNetwork_2Layer(inputSize=IMAGE_SIZE, outputSize=NUM_CLASSES, 
                                            neuronsPerLayer=neurons, learningRate = 0.5, activation = "sigmoid")
